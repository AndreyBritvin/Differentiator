...
INCLUDE = -Iinclude -IMy_logging_system/include -Iinclude/tree -IStack/include
...
SRC_FILES     = $(wildcard src/*.cpp) $(wildcard src/tree/*.cpp)
BUILD_FILES   = $(wildcard build/*.o)

MY_LIBS = My_logging_system/log_lib.a

all: differenciator.out

differenciator.out:$(SRC_FILES) $(BUILD_FILES)
	@$(CC) $(CFLAGS) $(INCLUDE) $(SRC_FILES) $(MY_LIBS) -o differenciator.out
